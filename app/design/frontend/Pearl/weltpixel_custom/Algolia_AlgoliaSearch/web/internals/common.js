				...
				source =  {
					...
					templates: {
						...
						suggestion: function (hit, payload) {
							...
							// tempPrice = $.trim(hit.price.EUR.default_formated);
							// tempPrice = tempPrice.replace(',', '');
							// symbol = tempPrice.replace(tempPrice.match(/(\d+)/g)[0], '').trim();
							// tempPrice = hit.price.EUR.default_formated.slice(0, -3);
							// hit.price.EUR.default_formated = tempPrice+' '+symbol;

                            if(typeof hit.price !== 'undefined'
								&& typeof hit.price.EUR !== 'undefined'
                                && typeof hit.price.EUR.default_formated !== 'undefined') {
                                let temporalPrice = $.trim(hit.price.EUR.default_formated);
                                const prices = temporalPrice.split("-");

                                if(typeof hit.price.EUR.default !== 'undefined' &&
									typeof prices !== 'undefined' && prices.length > 1 ) {
									// hit.price.EUR.default_formated = "Desde: " + priceUtils.formatPrice(hit.price.EUR.default);
									let priceString = hit.price.EUR.default;
									hit.price.EUR.default_formated = "Desde: " + priceString.toString().replace('.', ',') + " €";
                                }
							}
