...
define([
    ...
    'Firebear_ConfigurableProducts/js/action/update-stock',
    'jquery/ui',
    'Magento_Swatches/js/swatch-renderer'
], function ($, _, $translate, urlBuilder, priceUtils, updateStock) {
    'use strict';
    ...
    $.widget('mage.SwatchRenderer', $.mage.SwatchRenderer, {
        ...
        _EventListener: function () {
            ...
            $widget.element.on('click', '.' + this.options.classes.optionClass, function () {
                if(($(this).parent('td')).length > 0) { //Todo: that means is inside a matrix table
                    return null;
                } else {
                    return $widget._OnClick($(this), $widget);
                }
            });
            /** this method will trigger the change product in the table tr instead of triggered in the swatch */
            $widget.element.on('click', '.' + this.options.classes.optionClass + '-tr', function (e) {
                var swatchElement = $(this).find('.'+$widget.options.classes.optionClass);
                var targetElement = $(e.target);
                /** Looking for the element swatch to trigger the element of click and also verified
                 * the target is not an button or a input text
                 * */
                if(swatchElement.length > 0  && !targetElement.hasClass('arrow-matrix')
                    && !targetElement.hasClass('input-text') ) {

                    return $widget._OnClick(swatchElement, $widget);
                }
                return null;
            });
        ...
        _ReplaceData: function (simpleProductId, $widget) {
            if($widget.options.jsonConfig.hasOwnProperty('saleableProducts')
                && !this._isInStock($widget.options.jsonConfig.saleableProducts, simpleProductId)) {
                this._updateAvailability(simpleProductId, false);
            } else {
                this._updateAvailability(simpleProductId, true);
            }
            if($widget.options.jsonConfig.hasOwnProperty('stockExpress')) {
                if(this._hasStockExpress($widget.options.jsonConfig.stockExpress, simpleProductId)) {
                    this._updateStockExpress(true);
                } else {
                    this._updateStockExpress(false);
                }
            } else {
                this._updateStockExpress(false);
            }
            //Todo: Fix to calculate the prices again
            if(typeof $widget.options.jsonConfig.optionPrices[simpleProductId] !== 'undefined'){
                var pricesArray = $widget.options.jsonConfig.optionPrices[simpleProductId];
                var htmlPrice = '<span class="price-container ">';
                if(pricesArray.pvpDiscount ) {
                    const price = pricesArray.pvpDiscount.amount;
                    $('.product.pvp_discount').addClass("visible");
                    $('.product.pvp_discount .discountPvp').html("-" + parseFloat(price).toFixed(0) + "%");
                }else{
                    $('.product.pvp_discount .discountPvp').removeClass("visible").html("");
                }
                if(pricesArray.finalPrice) {
                    const price = pricesArray.finalPrice.amount;
                    htmlPrice = htmlPrice + '<span class="regular-price"><span class="price">'+this.getFormattedPrice(price, $widget)+'</span></span>';
                }
                if(pricesArray.oldPrice) {
                    const price = pricesArray.oldPrice.amount;
                    htmlPrice = htmlPrice + '<span class="final-price"><span class="price">'+this.getFormattedPrice(price, $widget)+'</span></span>';
                }

                htmlPrice = htmlPrice + '</span>';
                $('.price-box[data-role="priceBox"]').html(htmlPrice);
            }
            ...
            $('.loader-wrapper').hide();
            $('div.product-add-form').removeClass('product-add-custom-wrapper');
        },
        ...
        _OnClick: function ($this, $widget) {
            ...
            let positionLeftSwatch = 0;
            if(typeof position !== 'undefined') {
                positionLeftSwatch = position.left + 17.5;
            }
            const lineColor = $('.swatch-attribute.orden_color .line-color');
            ...
            if ($this.hasClass('selected')) {
                if(lineColor.length > 0) {
                    lineColor.show();
                    lineColor.width(positionLeftSwatch);
                }
                return;
            ...
            let configurable_matrix = $('.product-options-wrapper .swatch-opt .swatch-attribute');

            if ( configurable_matrix.attr('attribute-code') === "volumen" ) {
                $('.price-box').hide();
            }
        },
        ...
        _ChangeFromToPrice: function (attributeId, optionId, $widget) {
            ...
            $.each($widget.options.jsonConfig.attributes[0].options, function ($key, $item) {
                if (optionId == 0) {
                    productOptions = 1;
            ...
            if (selectedAttributesForProducts.length != 0) {
                selectedAttributesForProducts.forEach(function (item, i, selectedAttributesForProducts) {
                    $.each($widget.options.jsonConfig.attributes[i].options, function ($key, $item) {
                        if ($item.id == item) {
                            productsIds[iteratorForProductIds] = $item.products;
                            iteratorForProductIds++;
                        }
        ...
        _RenderFormInput: function (config, $widget) {
            ...
            let lineColor = '';
            if(config.code == $widget.options.jsonConfig.customColorCode) {
                lineColor = '<div class="line-color" id="line-color-'+config.id+'"></div>'
            }
            return lineColor+'<input class="' + this.options.classes.attributeInput + ' super-attribute-select" ' +
         ...
         * @private
         */
        _RenderFormInputDefault: function (config, $widget) {
            let lineColor = '<div class="line-color" id="line-color-'+config.id+'"></div>';
        ...
        _RenderControls: function () {
            ...
            if ($widget.options.jsonConfig.attributes.length == 1) {
                /**
                 * #################### Problem with configurable product which don't have attribute Color #################
                 */
                if($widget.options.jsonConfig.customColorData.id) {
                ...
                } else if($widget.options.jsonConfig.customSizeData.id){
                    containerHtml = '<div class="' + classes.attributeClass + ' ' + $widget.options.jsonConfig.customSizeData.code +
                        '" attribute-code="' + $widget.options.jsonConfig.customSizeData.code +
                        '" attribute-id="' + $widget.options.jsonConfig.customSizeData.id + '">' + containerHtml + '</div>';
                }
            }
            if (sizeContainder != '') {
        ...
        /**
         * Update the status and the alert products
         */
        _updateAvailability: function(productId, isInStock) {
            //updateStock(productId);
            var buttonCart = $('.product-options-bottom .box-tocart');
        ...
        /**
         * Update the stock express button
         * @param hasStockExpress
         * @private
         */
        _updateStockExpress(hasStockExpress) {
            let stockExpressContent = $('#stock-express-button');
            if(stockExpressContent.length > 0) {
                if(hasStockExpress) {
                    stockExpressContent.show();
                } else {
                    stockExpressContent.hide();
                }
            }
        },
        /**
         * New customization to check whether a product is in stock or not.
         * @param saleableProducts
         * @param product
         * @returns {boolean}
         * @private
         */
        _isInStock: function(saleableProducts, product){
            for(var i in saleableProducts) {
                if(i === product && saleableProducts[i]) {
                    return true;
                }
            }
            return false;
         ...
         * @private
         */
        _hasStockExpress(stockProducts, product) {
            for(var i in stockProducts) {
                if(i === product) {
                    return stockProducts[i];
                }
            }
            return false;
        },
        ...
        _RenderSwatchOptions       : function (config, only_color) {
            ...
            if($widget.options.jsonConfig.hasOwnProperty('saleableProducts')) {
                saleableProducts = $widget.options.jsonConfig.saleableProducts;
            }
            var self = this;
            $.each(config.options, function () {
                var id,
                ...
                if ($widget.options.jsonConfig.attributes.length == 1) {
                    if (!only_color && type !== 1 || config.code == $widget.options.jsonConfig.customSizeData.code) {
                        html += '<tr class="'+$widget.options.classes.optionClass+'-tr">';
                    }
                }
                ...
                if (type === 0) {
                    ...
                    if ($widget.options.jsonConfig.customColorData.code == config.code) {
                        ...
                        var productId = optionsMap[id].products[0];
                        var pvpDiscountPer = 0;
                        if(typeof  $widget.options.jsonConfig.optionPrices[productId].pvpDiscount !== 'undefined') {
                            pvpDiscountPer = $widget.options.jsonConfig.optionPrices[productId].pvpDiscount.amount;
                            pvpDiscountPer = pvpDiscountPer.replace("%", "");
                        }
                        
                        var style = '';
                        if(!pvpDiscountPer){
                            var style = 'style="opacity: 0;"';
                            $('.product.pvp_discount').removeClass("visible").html("");
                        }
                        html += '<td class="discount_percentage_label_' + id + '"><div class="discount_percentage_label" '+ style +' >' + pvpDiscountPer + '%</div></td>';
                        html += '<td><div class="' + optionClass + ' text" ' + attr + '>' + (value ? value : label) + '</div></td>';
                    }
                } else if (type === 1) {
                    // Color
                    if ($widget.options.jsonConfig.attributes.length == 1) {
                ...
                if ($widget.options.jsonConfig.attributes.length == 1) {
                    if (!only_color && type !== 1 || config.code == $widget.options.jsonConfig.customSizeData.code) {
                        ...
                        var productid = optionsMap[id].products[0];
                        var specialPrice = 0;
                        if(typeof $widget.options.jsonConfig.optionPrices[productid].pvpPrice !== 'undefined') {
                            specialPrice = $widget.options.jsonConfig.optionPrices[productid].pvpPrice.amount;
                        }
                        var style = '';
                        if(!specialPrice){
                            var style = 'style="opacity: 0;"';
                        }
                        html += '<td class="special_price_' + id + '"><div class="special_price" '+ style +' >' + specialPrice.toFixed(2).replace(".", ",") + currencySymbol + '</div></td>';
                        html += '<td class="size-price size_price_' + id + '">' + optionsMap[id].price.toFixed(2).replace(".", ",") + currencySymbol + '</td>';
                         ...
                         */
                        if(this.hasOwnProperty('products') && this.products.length === 1 && !self._isInStock(saleableProducts, this.products[0])) {
                            html += '<td class="matrix_qty_td size_qty_' + id + '"><div class="firebear_qty_block out-of-stock">'+ $translate('Fuera de existencias')+'</div></td>';
                        } else {
                            html += '<td class="matrix_qty_td size_qty_' + id + '"><div class="firebear_qty_block"><span class="arrow-matrix dec_matrix_arrow inc_qty_' + id + '">&ndash;</span><input type="number" name="qty_matrix_product[' + config.id + '][' + id + ']" maxlength="12" value="0" title="Qty" class="qty input-text matrix_qty qty' + id + '" data-validate="{&quot;required-number&quot;:true}"><span class="arrow-matrix inc_matrix_arrow inc_qty_' + id + '">&plus;</span></div></td>';
                        }
                        html += '</tr>';
                    }
                }
                    ...
                    if (!only_color && type !== 1 && type !== 2) {
                        if ($widget.options.jsonConfig.customColorData.code != config.code) {
                            ...
                            //     html += '<td class="size_stock_' + id + '">' + optionsMap[id].qty + '</td>';
                            // }
                            // html += '<td class="size_available_' + id + '">' + $translate("In stock") + '</td>';
                            var productid = optionsMap[id].products[0];
                            var specialPrice = $widget.options.jsonConfig.optionPrices[productid].pvpPrice.amount;
                            if(!specialPrice){
                                var style = 'style="opacity: 0;"';
                            }
                            html += '<td class="special_price_' + id + '"><div class="special_price" '+ style +' >' + specialPrice.toFixed(2).replace(".", ",") + currencySymbol + '</div></td>';
                            ...
                            if ($widget.options.jsonConfig.tierPrice) {
                                ...
                                $.each(tierPriceQtyArrayUnique, function (key, from) {
                                    if (optionsMap[id].tierPrice && optionsMap[id].tierPrice.price[from]) {
                                        html += '<td class="matrix_qty_val_from_' + from + '_' + id + ' tier_price_td">' + optionsMap[id].tierPrice.price[from].toFixed(2) + currencySymbol + '</td>';
                                    }
                                    else {
                                        html += '<td class="matrix_qty_val_from_' + from + '_' + id + ' tier_price_td">' + $translate('not set') + '</td>';
                                    }
        ...
        _RenderSwatchOptionsDefault: function (config, controlId) {
            var optionConfig = this.options.jsonSwatchConfig[config.id],
                ...
                countAttributes = 0,
                html = '';
            var jsConfig = this.options.jsonConfig;
            if (!this.options.jsonSwatchConfig.hasOwnProperty(config.id)) {
                return '';
            }
            ...
            $.each(config.options, function () {
                ...
                value = optionConfig[id].hasOwnProperty('value') ? optionConfig[id].value : '';
                thumb = optionConfig[id].hasOwnProperty('thumb') ? optionConfig[id].thumb : '';
                if(this.products && jsConfig.hasOwnProperty('swatches')) {
                    for(const index in this.products) {
                        const productId = this.products[index];
                        for(const swatchIndex in jsConfig.swatches) {
                            if(productId === swatchIndex) {
                                value = jsConfig.swatches[swatchIndex].url;
                                thumb = jsConfig.swatches[swatchIndex].url;
                                type = 2;
                                break;
                            }
                        }
