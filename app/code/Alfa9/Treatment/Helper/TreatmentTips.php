...
class TreatmentTips extends AbstractHelper
{
    /**
    ...
    public function getTreatmentTips($today = null)
    {
        $tipsEnabled = $this->scopeConfig->getValue('pss_treatments/general/enable_tips');

        if($tipsEnabled){
            $currentDate = ($today) ? date('Y-m-d', strtotime($today)) : date('Y-m-d');

            $daysToTip = $this->scopeConfig->getValue('pss_treatments/general/days_to_tips');
            $tipsDate = date('Y-m-d', strtotime($currentDate . ' - ' . $daysToTip . ' days'));

            $treatmentDates = $this->treatmentFactory->create()->getCollection()
                ->addFieldToFilter('delivery_days', $tipsDate);
