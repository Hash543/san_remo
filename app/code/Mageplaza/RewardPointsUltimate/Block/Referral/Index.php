        ...
        if ($rule->getReferralType() == ReferralActions::TYPE_PRICE) {
            $this->setReferMessage(
                __(
                    ...
                    $pointHelper->format($rule->getReferralPoints()),
                    $rule->getReferralMoneyStep()
                )
            );
        } else {
            $this->setReferMessage(
                __('You will receive %1 purchasing made by your friends.'),
                $pointHelper->format($rule->getReferralPoints())
            );
        }
        ...
        switch ($customerAction) {
            case CustomerActions::TYPE_FIXED_POINTS:
                $this->setCustomerMessage(
                    __('Your friends will be received %1 when purchasing at our store.'),
                    $pointHelper->format($rule->getCustomerPoints())
                );
                break;
