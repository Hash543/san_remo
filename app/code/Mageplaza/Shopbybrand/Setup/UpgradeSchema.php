        ...
        if (version_compare($context->getVersion(), '2.2.1', '<')) {
            ...
            if (!$installer->tableExists('mageplaza_shopbybrand_brand_category')) {
                $table = $installer->getConnection()
                    ...
                    ->addColumn('option_id', Table::TYPE_INTEGER, null, ['nullable' => false, 'unsigned' => true, 'primary' => true])
                    ->addColumn('position', Table::TYPE_INTEGER, null, ['nullable' => false, 'default' => 0])
                    ->addIndex($installer->getIdxName('mageplaza_shopbybrand_brand_category', ['option_id']), ['option_id'])
            ...
            if ($connection->tableColumnExists('mageplaza_brand', 'is_premium') === false) {
                $connection->addColumn(
                    $installer->getTable('mageplaza_brand'),
                    'is_premium',
                    [
                        'type' => Table::TYPE_INTEGER,
                        'length' => 11,
                        'nullable' => true,
                        'comment' => 'Brand Premium Brand',
                        'after' => 'is_featured'
                    ]
                );
            }

            if ($connection->tableColumnExists('mageplaza_brand', 'premium_landing_block') === false) {
                $connection->addColumn(
                    $installer->getTable('mageplaza_brand'),
                    'premium_landing_block',
                    [
                        'type' => Table::TYPE_TEXT,
                        'length' => null,
                        'nullable' => true,
                        'comment' => 'Premium Brand Landing Block'
                        
                    ]
                );
            }

            if ($connection->tableColumnExists('mageplaza_brand', 'premium_sidebar_block') === false) {
                $connection->addColumn(
                    $installer->getTable('mageplaza_brand'),
                    'premium_sidebar_block',
                    [
                        'type' => Table::TYPE_TEXT,
                        'length' => null,
                        'nullable' => true,
                        'comment' => 'Premium Brand Sidebar Block'
