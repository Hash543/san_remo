    ...
    $.widget('mage.ampromoPopup', {
        ...
        reload: function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('.ampromo-popup-header').css('max-width', (itemsCount >= 3 ? 3 * 280 : itemsCount * 280) + 'px');
                ...
                $.each(rulesData['sku'], function (index, value) {
                    ...
                    if (productDomBySku) {
                        ...
                        var qtyInput = $(productDomBySku.find('[data-role=ampromo-item-qty-input]')
                            .find('.ampromo-item-qty-select-input'));
                        if (qtyInput.length) {
        ...
        checkboxState: function (elem) {
            ...
            if (!($(elem).attr('checked'))) {
                $(elem.parentElement.parentElement).find('.ampromo-item-qty-select-input')[0].value = 0;
                $(elem.parentElement.parentElement).find('.ampromo-item-qty-select-input').keyup();
            }

            if ($(elem).attr('checked')) {
                $(elem.parentElement.parentElement).find('.ampromo-item-qty-select-input')[0].value = 1;
                $(elem.parentElement.parentElement).find('.ampromo-item-qty-select-input').keyup();
            }
        ...
        getRuleId: function (elem) {
            return this.getProductDomByElem(elem).find('.ampromo-item-qty-select-input').attr('data-rule');
        },
        ...
        getRuleType: function (elem) {
            return this.getProductDomByElem(elem).find('.ampromo-item-qty-select-input').attr('data-rule-type');
        },
