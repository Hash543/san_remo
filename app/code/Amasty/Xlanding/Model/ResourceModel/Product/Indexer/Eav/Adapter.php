...
namespace Amasty\Xlanding\Model\ResourceModel\Product\Indexer\Eav;
...
class Adapter extends AbstractEav
{
    ...
    {
        $this->prepareSelectIndex($entityIds, $attributeId);
        $this->prepareMultiselectIndex($entityIds, $attributeId);
     ...
     * @return array
     */
    private function getIndexableAttributes($multiSelect)
    {
     ...
     */
    private function prepareSelectIndex($entityIds = null, $attributeId = null)
    {
        $connection = $this->getConnection();
        $idxTable = $this->getIdxTable();
        // prepare select attributes
        $attrIds = $attributeId === null ? $this->getIndexableAttributes(false) : [$attributeId];
        if (!$attrIds) {
            return $this;
        }
     ...
     * @return $this
     */
    private function prepareMultiselectIndex($entityIds = null, $attributeId = null)
    {
        $connection = $this->getConnection();

        // prepare multiselect attributes
        $attrIds = $attributeId === null ? $this->getIndexableAttributes(true) : [$attributeId];
    ...
    /**
     * Copy full function because it defined differently in magento versions
     * @inheritdoc
     */
