...
class Page extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function checkIdentifier($identifier, $storeId)
    {
        ...
        $select = $this->_getLoadByIdentifierSelect($identifier, $stores, [PageModel::STATUS_DYNAMIC, PageModel::STATUS_ACTIVE]);
    ...
    public function getIndexPageIdsByProductId($productIds, $storeId = null)
    {
        ...
        $select = $connection->select()->from(
            ['main_table' => $this->getTable($table)],
            [ 'product_id', 'page_ids'=> new \Zend_Db_Expr("GROUP_CONCAT(page_id separator ',')")]
        )->where(
            'store_id = ?', $storeId
        )->where(
            'product_id IN (?)', $productIds
        )->group('product_id');
        $positionData = $connection->fetchPairs($select);

        foreach ($positionData as $productId => $pageIds) {
            $positionData[$productId] = explode(',', $pageIds);
        }
