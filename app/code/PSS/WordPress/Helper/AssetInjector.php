...
use PSS\WordPress\Model\Url as WordPressURL;
...
class AssetInjector
{
	...
	public function __construct(
		...
		$this->moduleVersion      = $moduleList->getOne('PSS_WordPress')['setup_version'];
...
#			return preg_replace('/\.js$/', '', preg_replace('/\?.*$/', '', $externalScriptUrlFull));
	...
	{
		$scriptContent = preg_replace('/[a-zA-Z$]{1,}\(document\)\.ready\(/', 'FPJS.on(\'wordpress_ready\', ', $scriptContent);
		$scriptContent = preg_replace('/jQuery\([\s]{0,}function\(/i', 'FPJS.on(\'wordpress_ready\', function(', $scriptContent);
    ...
    /**
     *  Given a URL, check for define.AMD and if found, rewrite file and disable this functionality
     * @param array $externalScriptUrlFulls
     * @return string
     * @throws \Magento\Framework\Exception\FileSystemException
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
