 ...
 * To customise things, create a file at wp-content/themes/wordpress/local.php
...
#		add_filter('preview_post_link',  array($this, 'onFilterPreviewPostLink'), 10, 2);
	...
	{
		if ($data = get_option('pss_magento')) {
	...
	public function invalidateMagento2FPC($post_id)
	{
		...
		$salt = get_option( 'pss_salt' );
		
		if (!$salt) {
			$salt = wp_generate_password( 64, true, true );
			update_option( 'pss_salt', $salt );
		}
		...
		$nonce = substr( hash_hmac( 'sha256', $nonce_tick . '|pss|' . $action, $salt ), -12, 10 );
	...
	public function onActionAddMetaBoxesRelatedProducts()
	{
		add_meta_box(
			...
			function() {
				?>
				<p>Link your WordPress posts (any post type) with Magento products using the new <a href="https://pss.co.uk/magento/wordpress-integration/related-products/" target="_blank">Related Products</a> module by pss.</p>
				<button onclick="window.open('https://pss.co.uk/magento/wordpress-integration/related-products/');" type="button" class="components-button is-button is-default is-primary is-large">View Module</button>
				<?php
			}
		);
	}
