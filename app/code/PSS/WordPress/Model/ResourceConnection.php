...
use PSS\WordPress\Model\Logger;
...
class ResourceConnection {
	...
	{		
		...
		if ((int)$this->storeManager->getStore()->getId() > 0) {
			$optionName  = 'pss_magento_base_url';
			$optionTable = $this->getTable('wordpress_option');
			$baseUrl     = $this->storeManager->getStore()->getBaseUrl();

			try {
				$this->_setOptionValue('pss_magento_base_url', $this->storeManager->getStore()->getBaseUrl());
				$this->_setOptionValue('pss_magento_version',  '2');

				$this->_setOptionValue('pss_magento', json_encode([
