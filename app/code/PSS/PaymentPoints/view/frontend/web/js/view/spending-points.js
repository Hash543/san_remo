...
define([
    ...
    'mage/translate',
    'mpIonRangeSlider'
], function ($, ko, _, Component, spendingPoints, customerData, points, quote, $t) {
    'use strict';
    ...
    return Component.extend({
        defaults: {
            template: 'PSS_PaymentPoints/spending-points'
        },
        ...
        balanceFormatted: ko.computed(function () {
            var label = $t('Canjear puntos, saldo actual %s.');
        ...
        initialize: function () {
            ...
            let automaticallyChangePoints = false;
            if( priceFormat.pattern.indexOf("SRP") !== -1) {
                automaticallyChangePoints = true;
            }
            this.canSpendPoints = ko.computed(function () {
                return !$.isEmptyObject(spendingConfig()) && !automaticallyChangePoints
                    && typeof this.rules() !== "undefined" && this.rules().length && this.selectedRule();
            }, this);
        ...
        calculatePoints: function(){
            var rule = this.selectedRule();
            var value = parseInt($('#calculate-points').val());
            var newValue = (value < rule.min) ? rule.min : ((value > rule.max) ? rule.max : value);
