...
class Validator extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        $errors = null;
        if (!isset($values['firma_local']) || !isset($values['firma_remota'])
            || $values['firma_local'] != $values['firma_remota']) {
        ...
        if (!isset($values['codigo']) || !isset($values['codigo_orig'])
            || !$this->checkFuc($values['codigo']) || $values['codigo'] != $values['codigo_orig']) {
        ...
        if (!isset($values['tipo_trans']) || !isset($values['tipo_trans_orig'])
            || $values['tipo_trans'] != $values['tipo_trans_orig']) {
            $errors[] = 'Problemas con la firma local y remota. No coinciden o no existen.';
        }
        if (!isset($values['terminal']) || !isset($values['terminal_orig'])
            || $values['terminal'] != $values['terminal_orig']) {
    ...
    private function checkFuc($codigo)
    {
        ...
        if ($retVal) {
            ...
            for ($i = strlen($fucTemp) - 1; $i >= 0; $i-=2) {
                $temp = (int)(substr($fucTemp, $i, 1)) * 2;
                $acumulador += (int)($temp / 10) + ($temp % 10);
                if ($i > 0) {
                    $acumulador += (int)(substr($fucTemp, $i - 1, 1));
                }
            }
