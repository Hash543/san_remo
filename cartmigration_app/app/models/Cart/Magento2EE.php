...
class LECM_Model_Cart_Magento2EE
...
//        $this->_notice['support']['cartrules']          = true;
    ...
    public function displayConfigTarget()
    {
        ...
        $response       = $this->_defaultResponse();
        $default_config = $this->getConnectorData($this->getConnectorUrl('query', null, 'target'), array(
            'serialize' => true,
            'query'     => serialize(array(
                'languages'       => array(
                    'type'  => 'select',
        ...
        $default_config_data = $default_config['data'];
        if ($default_config_data && $default_config_data['languages'] && $default_config_data['currencies'] && $default_config_data['eav_entity_type']) {
            $this->_notice['target']['language_default'] = $this->_getDefaultLanguage($default_config_data['languages']);
            $this->_notice['target']['currency_default'] = isset($default_config_data['currencies'][0]['value']) ? $default_config_data['currencies'][0]['value'] : 'USD';
            foreach ($default_config_data['eav_entity_type'] as $eav_entity_type) {
                $this->_notice['target']['extends'][$eav_entity_type['entity_type_code']] = $eav_entity_type['entity_type_id'];
            }
        }
        $this->_notice['target']['category_root'] = 1;
...
//        );
        ...
        $config = $this->getConnectorData($this->getConnectorUrl('query', null, 'target'), array(
            ...
            'query'     => serialize(array(
                ...
                'attributes_set' => array(
                    'type'  => 'select',
                    'query' => "SELECT * FROM _DBPRF_eav_attribute_set WHERE entity_type_id = 4"
                ),
                "category_root"  => array(
                    'type'  => 'select',
                    'query' => "SELECT a.entity_id, b.value FROM _DBPRF_catalog_category_entity a, _DBPRF_catalog_category_entity_varchar b, _DBPRF_eav_attribute c
                ...
                "categories"     => array(
                    'type'  => 'select',
                    'query' => 'SELECT COUNT(1) AS count FROM _DBPRF_catalog_category_entity'
                ),
            )),
        ));
        ...
        $config_data   = $config['data'];
        $language_data = $currency_data = $order_status_data = $customer_group_data = array();
        $langexist     = array();
        foreach ($default_config_data['languages'] as $language_row) {
            $lang_id                                   = $language_row['store_id'];
            $lang_name                                 = $language_row['name'];
            $website_id                                = $language_row['website_id'];
            $this->_notice['target']['site'][$lang_id] = $website_id;
            if (!in_array($lang_name, $langexist)) {
                $langexist[]             = $lang_name;
                $language_data[$lang_id] = $lang_name;
            }
        ...
        if (count($category_root_data) > 0) {
            $this->_notice['target']['categoryData'] = $category_root_data;
        }
        ...
        $this->_notice['target']['support']['language_map'] = true;
        // $this->_notice['target']['languages'] = $language_data;
        $this->_notice['target']['support']['order_status_map'] = true;
        // $this->_notice['target']['order_status'] = $order_status_data;
        $this->_notice['target']['support']['currency_map'] = true;
        // $this->_notice['target']['currencies'] = $currency_data;
        $this->_notice['target']['support']['country_map'] = false;//
        ...
        $this->_notice['target']['support']['taxes']              = true;
        $this->_notice['target']['support']['manufacturers']      = true;
        ...
        $this->_notice['target']['support']['reviews']            = true;
...
//        $real_totals['products'] = 1;
    ...
    protected function _clearTargetCategories()
    {
        ...
        $root_category_ids = array();

        $root_category_data = $this->_notice['target']['categoryData'];
        foreach ($root_category_data as $key => $value) {
            $root_category_ids[] = $key;
        }

        $root_category_ids[] = 1;

        $root_ids_in_condition = $this->arrayToInCondition($root_category_ids);
        $table_index           = $this->_notice['target']['clear']['table_index'];
        if (isset($tables[$table_index])) {
            $this->_notice['target']['clear']['result']   = 'process';
            $this->_notice['target']['clear']['function'] = '_clearTargetCategories';
            $table                                        = $tables[$table_index];
            $where                                        = ' WHERE row_id NOT IN ' . $root_ids_in_condition;
...
//        }
    ...
    public function getCategoriesExtExport($categories)
    {
        ...
        $rowIds            = $this->duplicateFieldValueFromList($categories['data'], 'row_id');
        $category_id_query      = $this->arrayToInCondition($categoryIds);
        $row_id_query      = $this->arrayToInCondition($rowIds);
        $categories_ext_queries = array(
            'catalog_category_entity_varchar'  => array(
                'type'  => 'select',
                'query' => "SELECT * FROM _DBPRF_catalog_category_entity_varchar WHERE entity_id IN " . $row_id_query
            ),
            'catalog_category_entity_text'     => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_category_entity_text WHERE entity_id IN {$row_id_query}"
            ),
            'catalog_category_entity_int'      => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_category_entity_int WHERE entity_id IN {$row_id_query}"
            ),
            'catalog_category_entity_decimal'  => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_category_entity_decimal WHERE entity_id IN {$row_id_query}",
            ),
            'catalog_category_entity_datetime' => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_category_entity_datetime WHERE entity_id IN {$row_id_query}"
            ),
            'eav_attribute'                    => array(
                'type'  => "select",
    ...
    public function convertCategoryExport($category, $categoriesExt)
    {
        ...
        $entity_varchar = $this->getListFromListByField($categoriesExt['data']['catalog_category_entity_varchar'], 'row_id', $category['row_id']);
        $entity_text    = $this->getListFromListByField($categoriesExt['data']['catalog_category_entity_text'], 'row_id', $category['row_id']);
        $entity_int     = $this->getListFromListByField($categoriesExt['data']['catalog_category_entity_int'], 'row_id', $category['row_id']);
...
//            }
        ...
        $insert_query = $this->createInsertQuery('sequence_catalog_category',array('sequence_value'=>''));
        $category_id = $this->importCartRuleData($insert_query);
        if(!$category_id){
            return $this->errorConnector();
        }
        $category_data  = array(
            ...
            'attribute_set_id' => 3,
            'parent_id'        => $parent_id,
            'created_at'       => $convert['created_at'] ? $this->convertStringToDatetime($convert['created_at']) : date("Y-m-d h:i:s"),
            'updated_at'       => ($convert['updated_at']) ? $this->convertStringToDatetime($convert['updated_at']) : date("Y-m-d h:i:s"),
            'path'             => $cate_path,
            'position'         => isset($convert['category']['position']) ? $convert['category']['position'] : 0,
            'level'            => isset($convert['level']) ? $convert['level'] : 0,
            'children_count'   => isset($convert['category']['children_count']) ? $convert['category']['children_count'] : 0,
        );

        $category_id = $this->importCategoryData($this->createInsertQuery('catalog_category_entity',$category_data));
...
//        }
    ...
    public function afterCategoryImport($category_id, $convert, $category, $categoriesExt)
    {
        ...
        $all_query = array();
        $url_query                      = $this->getConnectorUrl('query');
        $category_eav_attribute_queries = array(
            'eav_attribute' => array(
                'type'  => "select",
        ...
        $category_eav_attribute = $this->getConnectorData($url_query, array(
            'serialize' => true,
            'query'     => serialize($category_eav_attribute_queries)
        ));

        $category_eav_attribute_array = array();
        foreach ($category_eav_attribute['data']['eav_attribute'] as $attribute) {
            if ($attribute['backend_type'] != 'static') {
                $category_eav_attribute_array[$attribute['attribute_code']]['attribute_id'] = $attribute['attribute_id'];
                $category_eav_attribute_array[$attribute['attribute_code']]['backend_type'] = $attribute['backend_type'];
            }
        }
        ...
        // $categories_image = ' ';
        $image_name = null;
        if ($convert['image']['url'] && $convert['image']['path']) {
            if (Bootstrap::getConfig('migrate_image')) {
                $image_process = $this->processImageBeforeImport($convert['image']['url'], $convert['image']['path']);
                $image_import  = $this->getConnectorData($url_image, array(
                    'images' => serialize(array(
                        'ci' => array(
                            'type'   => 'download',
                            'path'   => $this->addPrefixPath(basename($image_process['path']), $this->_notice['target']['config']['image_category']),
                            'params' => array(
                ...
                if ($image_import && $image_import['result'] == 'success') {
                    ...
                    if ($image_import_path) {
                        ...
                        $image_name = basename($convert['image']['path']);
                    }
                }
            } else {
                $image_name = $convert['image']['path'];
            }

        }
        //image_end
        $url_key_multistore = array();
        $cate_url_key = $this->getValue($convert, 'url_key');
        $cate_name = $this->getValue($convert, 'name');
        if ($cate_url_key) {
            $cate_url_key = $this->getCategoryUrlKey($cate_url_key, 0, $category_eav_attribute_array['url_key']['attribute_id'],$cate_name);
        }

        $url_key_multistore[0] = $cate_url_key;
        $cate_url_path = $this->getCategoryUrlPath($convert['url_path'],0,$category_eav_attribute_array['url_path']['attribute_id'],$cate_url_key);
        $insert_attribute_array = array(
            ...
            'display_mode'               => $this->getValue($convert, 'display_mode'),
            'page_layout'                => null,
            'path'                       => null,
            'path_in_store'              => null,
            'position'                   => null,
            'url_key'                    => $cate_url_key ? $cate_url_key : null,
            'url_path'                   => $cate_url_path,
            'is_anchor'                  => $this->getValue($convert, 'is_anchor'),
        );
//        print_r($insert_attribute_array);exit;
        foreach ($category_eav_attribute_array as $key1 => $value1) {
            foreach ($insert_attribute_array as $key2 => $value2) {
                if ($key2 == $key1) {
                    ...
                    $category_attr_query = $this->createInsertQuery("catalog_category_entity_" . $value1['backend_type'],$category_attr_data);
                    if (Bootstrap::getConfig('dev_mode')) {
                        $category_attr_import = $this->importCategoryData($category_attr_query);

                        if (!$category_attr_import) {
                            Bootstrap::logQuery($category_attr_query);
                            var_dump(1);
                            exit;
...
//                            return $this->errorConnector();
        ...
        //language begin
        if (isset($convert['languages'])) {
            foreach ($convert['languages'] as $key => $value) {
                $insert_attribute_array = array(
                    'name'             => $value['name'],
                    ...
                    'display_mode'     => $this->getValue($value, 'display_mode'),
                    'url_key'          => $this->getValue($value, 'url_key'),
                    'url_path'         => $this->getValue($value, 'url_path'),
                    'is_anchor'        => $this->getValue($value, 'is_anchor'),
                );

                foreach ($category_eav_attribute_array as $key1 => $value1) {
                    foreach ($insert_attribute_array as $key2 => $value2) {
                        if ($key2 == $key1) {
                            $store_id = $this->getMapStoreView($key);
                            if ($key2 == 'url_key') {
                                $value2 = $this->getCategoryUrlKey($value2, $store_id, $value1['attribute_id'],$value['name']);
                                $url_key_multistore[$store_id] = $value2;
                            }
                            if ($key2 == 'url_path') {
                                $value2 = $this->getCategoryUrlPath($value2, $store_id, $value1['attribute_id']);
                            }
                            if(!$value2){
                                continue;
                            }
...
//            $url_rewrite = $model_seo->getCategoriesSeoExport($this, $category, $categoriesExt);
        ...
        foreach ($url_rewrite as $key => $rewrite) {
            ...
            $url_rewrite_data  = array(
                ...
                'entity_id'        => $category_id,
                'request_path'     => $path,
                'target_path'      => 'catalog/category/view/id/' . $category_id,
                'redirect_type'    => 0,
                'store_id'         => $store_id,
...
//        $value_id_query = $this->arrayToInCondition($valueIds);
        ...
        $allRowIds = $this->duplicateFieldValueFromList($productsExt['data']['catalog_product_entity'], 'row_id');
        $all_row_id_con = $this->arrayToInCondition($allRowIds);

        $product_ext_rel_rel_queries = array(
            'catalog_product_entity_tier_price'      => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_product_entity_tier_price WHERE row_id IN {$all_row_id_con}"
            ),
            'catalog_product_entity_datetime'        => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_product_entity_datetime WHERE row_id IN {$all_row_id_con}"
            ),
            'catalog_product_entity_decimal'         => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_product_entity_decimal WHERE row_id IN {$all_row_id_con}"
            ),
            'catalog_product_entity_gallery'         => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_product_entity_gallery WHERE row_id IN {$all_row_id_con}"
            ),
            'catalog_product_entity_int'             => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_product_entity_int WHERE row_id IN {$all_row_id_con}"
            ),
            'catalog_product_entity_text'            => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_product_entity_text WHERE row_id IN {$all_row_id_con}"
            ),
            'catalog_product_entity_varchar'         => array(
                'type'  => "select",
                'query' => "SELECT * FROM _DBPRF_catalog_product_entity_varchar WHERE row_id IN {$all_row_id_con}"
            ),
    ...
    public function convertProductExport($product, $productsExt)
    {
        ...
        $entity_decimal        = $this->getListFromListByField($productsExt['data']['catalog_product_entity_decimal'], 'row_id', $product['row_id']);
        $entity_int            = $this->getListFromListByField($productsExt['data']['catalog_product_entity_int'], 'row_id', $product['row_id']);
        $entity_text           = $this->getListFromListByField($productsExt['data']['catalog_product_entity_text'], 'row_id', $product['row_id']);
        $entity_varchar        = $this->getListFromListByField($productsExt['data']['catalog_product_entity_varchar'], 'entity_id', $product['row_id']);
        $entity_datetime       = $this->getListFromListByField($productsExt['data']['catalog_product_entity_datetime'], 'row_id', $product['row_id']);
        ...
        $product_media_ids              = $this->getListFromListByField($productsExt['data']['catalog_product_entity_media_gallery_value_to_entity'], 'row_id', $product['row_id']);
        ...
        $tiers_price = $this->getListFromListByField($productsExt['data']['catalog_product_entity_tier_price'], 'row_id', $product['row_id']);
        if ($tiers_price) {
...
//            }
        ...
        foreach ($url_rewrite as $rewrite) {
            ...
            if($is_category){
                ...
                $product_data['url_rewrite_product'] = $rewrite_data;
...
//    }
    ...
    public function productImport($convert, $product, $productsExt)
    {
        ...
        $url_src   = $this->_notice['src']['cart_url'];
        $url_desc  = $this->_notice['target']['cart_url'];
        $insert_query = $this->createInsertQuery('sequence_product',array('sequence_value'=>$this->_notice['config']['real_pre_prd']?$convert['id']:''));
        $product_id = $this->importCartRuleData($insert_query);
        if(!$product_id){
            return $this->errorConnector();
        }
        ...
        $catalog_product_entity_data = array(
            ...
            'created_in' => 1,
            'updated_in' => '2147483647',
            'attribute_set_id' => $this->_notice['map']['attributes'][$convert['attribute_set_id']],
            'type_id'          => $convert['type_id'] ? $convert['type_id'] : 'simple',
            'sku'              => $convert['sku'],
            'has_options'      => 0,
            'required_options' => 0,
...
//        }
        $catalog_product_entity_query = $this->createInsertQuery('catalog_product_entity',$catalog_product_entity_data);
...
//            $response['msg']    = $this->warningImportEntity('Product', $convert['id'], $convert['code']);
    ...
    public function afterProductImport($product_id, $convert, $product, $productsExt)
    {
            ...
            foreach ($convert['images'] as $key => $item) {
                if ($item['url'] && $item['path']) {
                    ...
                    if ($item_image_name) {
                        ...
                        $value_id = $this->importProductData($this->createInsertQuery('catalog_product_entity_media_gallery',$catalog_product_entity_media_gallery_data));
                        if($value_id){
                            ...
                            $catalog_product_entity_media_gallery_value_to_entity_query = $this->createInsertQuery('catalog_product_entity_media_gallery_value_to_entity',$catalog_product_entity_media_gallery_value_to_entity_data);
                            if (Bootstrap::getConfig('dev_mode')) {
                            ...
                            $catalog_product_entity_media_gallery_value_data = array(
                                'value_id'  => $value_id,
                                'store_id'  => 0,
                                'row_id' => $product_id,
                                'label'     => $this->getValue($item,'label'),
                                'position'  => $this->getValue($item,'position',1),
                                'disabled'  => 0,
                            );

                            $catalog_product_entity_media_gallery_value_query = $this->createInsertQuery('catalog_product_entity_media_gallery_value',$catalog_product_entity_media_gallery_value_data);
                            if (Bootstrap::getConfig('dev_mode')) {
                                $catalog_product_entity_media_gallery_value_import = $this->importProductData($catalog_product_entity_media_gallery_value_query);
                                if (!$catalog_product_entity_media_gallery_value_import) {
                                    //warning
                                    Bootstrap::logQuery($catalog_product_entity_media_gallery_value_query);
                                    var_dump(1);
                                    exit;
                                ...
                                $all_query[] = $catalog_product_entity_media_gallery_value_query;
                            }
                        }
        ...
        // main image begin
        if (isset($convert['image']['url']) && $convert['image']['path'] && !$image_name) {
            if (Bootstrap::getConfig('migrate_image')) {
                $image_process = $this->processImageBeforeImport($convert['image']['url'], $convert['image']['path']);
                $image_import  = $this->getConnectorData($url_image, array(
                    'images' => serialize(array(
                ...
                if ($image_import && $image_import['result'] == 'success') {
                    ...
                    if ($image_import_path) {
                        // $categories_image = $this->removePrefixPath($image_import_path, $this->_notice['target']['config']['image_category']);
                        $image_name = $this->makeMagentoImagePath($convert['image']['path']) . basename($convert['image']['path']);
                    }
                }
            } else {
                $image_name = $convert['image']['path'];
            ...
            if ($image_name) {
                ...
                $catalog_product_entity_media_gallery_query  = $this->createInsertQuery('catalog_product_entity_media_gallery',$catalog_product_entity_media_gallery_data);
                $value_id = $this->importProductData($catalog_product_entity_media_gallery_query);

                if (!$value_id) {
                    //warning
                    if (Bootstrap::getConfig('dev_mode')) {
                        Bootstrap::logQuery($catalog_product_entity_media_gallery_query);
                        var_dump(1);
                        exit;
                    }
...
//                return $this->errorConnector();
                ...
                if($value_id){
                    ...
                    if (Bootstrap::getConfig('dev_mode')) {
                        ...
                        if (!$catalog_product_entity_media_gallery_value_to_entity_import) {
                            //warning
                            var_dump($catalog_product_entity_media_gallery_value_to_entity_query);
                            exit;
...
//=================================================================
        ...
        foreach ($convert['categories'] as $key => $value) {
            ...
            $catalog_category_product_query = $this->createInsertQuery('catalog_category_product',$catalog_category_product_data);
...
//            $catalog_category_product_index_data = array(
//                'category_id' => $category_id,
//                'product_id'  => $product_id,
//                'position'    => 0,
//                'is_parent'   => 0,
...
//            );
//
//            $catalog_category_product_index_query = $this->createInsertQuery('catalog_category_product_index',$catalog_category_product_index_data);
//
//            if (Bootstrap::getConfig('dev_mode')) {
//                $catalog_category_product_index_import = $this->importProductData($catalog_category_product_index_query);
//
//                if (!$catalog_category_product_index_import) {
//                    //warning
//                    Bootstrap::logQuery($catalog_category_product_index_query);
//                    var_dump(1);
//                    exit;
//                }
//
//            } else {
//                $all_query[] = $catalog_category_product_index_query;
//            }
...
//=================================================================
        //cataloginventory_stock (quality) begin

        //cataloginventory_stock_status

        $cataloginventory_stock_status_data = array(
            'product_id'   => $product_id,
            'website_id'   => 0,
            'stock_id'     => 1,
            'qty'          => $convert['qty'],
        ...
        $cataloginventory_stock_status_query = $this->createInsertQuery('cataloginventory_stock_status',$cataloginventory_stock_status_data);
        ...
        if (Bootstrap::getConfig('dev_mode')) {
            $cataloginventory_stock_status_import = $this->importProductData($cataloginventory_stock_status_query);

            if (!$cataloginventory_stock_status_import) {
                //warning
                Bootstrap::logQuery($cataloginventory_stock_status_query);
                var_dump(1);
                exit;
            }

        } else {
            $all_query[] = $cataloginventory_stock_status_query;
        }
         ...
         */
        //cataloginventory_stock_status_idx
        $cataloginventory_stock_status_idx_data = array(
            'product_id'   => $product_id,
            'website_id'   => 0,
            'stock_id'     => 1,
            'qty'          => $convert['qty'],
        ...
        $cataloginventory_stock_status_idx_query = $this->createInsertQuery('cataloginventory_stock_status_idx',$cataloginventory_stock_status_idx_data);


        if (Bootstrap::getConfig('dev_mode')) {
            $cataloginventory_stock_status_idx_import = $this->importProductData($cataloginventory_stock_status_idx_query);

            if (!$cataloginventory_stock_status_idx_import) {
                //warning
                Bootstrap::logQuery($cataloginventory_stock_status_idx_query);
                var_dump(1);
                exit;
            }

        } else {
            $all_query[] = $cataloginventory_stock_status_idx_query;
        }
         ...
         */
        // cataloginventory_stock_item
//        if(isset($convert['manage_stock'])){
//
//            var_dump($convert['manage_stock_data']);
//            exit;
//        }
        $manage_stock_data = $this->getValue($convert, 'manage_stock_data');
        if ($manage_stock_data) {
            $low_stock_date = $this->getValue($manage_stock_data, 'low_stock_date');
            if ($low_stock_date && $low_stock_date == '0000-00-00 00:00:00') {
                $low_stock_date = null;
            }

            $cataloginventory_stock_item_data = array(
                'product_id'                  => $product_id,
                'stock_id'                    => 1,
                'qty'                         => $this->getValue($convert, 'qty'),
                'min_qty'                     => $this->getValue($manage_stock_data, 'min_qty', '0.0000'),
                'use_config_min_qty'          => $this->getValue($manage_stock_data, 'use_config_min_qty', 1),
                'is_qty_decimal'              => $this->getValue($manage_stock_data, 'is_qty_decimal', 0),
                'backorders'                  => $this->getValue($manage_stock_data, 'backorders', 0),
                'use_config_backorders'       => $this->getValue($manage_stock_data, 'use_config_backorders', 1),
                'min_sale_qty'                => $this->getValue($manage_stock_data, 'min_sale_qty', '1.0000'),
                'use_config_min_sale_qty'     => $this->getValue($manage_stock_data, 'use_config_min_sale_qty', 1),
                'max_sale_qty'                => $this->getValue($manage_stock_data, 'max_sale_qty', '0.0000'),
                'use_config_max_sale_qty'     => $this->getValue($manage_stock_data, 'use_config_max_sale_qty', 1),
                'is_in_stock'                 => $this->getValue($manage_stock_data, 'is_in_stock', 0),
                'low_stock_date'              => $low_stock_date,
                'notify_stock_qty'            => $this->getValue($manage_stock_data, 'notify_stock_qty'),
                'use_config_notify_stock_qty' => $this->getValue($manage_stock_data, 'use_config_notify_stock_qty', 1),
                'manage_stock'                => $this->getValue($manage_stock_data, 'manage_stock', 0),
                'use_config_manage_stock'     => $this->getValue($manage_stock_data, 'use_config_manage_stock', 1),
                'stock_status_changed_auto'   => $this->getValue($manage_stock_data, 'stock_status_changed_auto', 0),
                'use_config_qty_increments'   => $this->getValue($manage_stock_data, 'use_config_qty_increments', 0),
                'qty_increments'              => $this->getValue($manage_stock_data, 'qty_increments', '0.0000'),
                'use_config_enable_qty_inc'   => $this->getValue($manage_stock_data, 'use_config_enable_qty_inc', 1),
                'enable_qty_increments'       => $this->getValue($manage_stock_data, 'enable_qty_increments', 0),
                'is_decimal_divided'          => $this->getValue($manage_stock_data, 'is_decimal_divided', 0),
                'website_id'                  => 0,
            );

//            if ($convert['qty'] < 1) {
//                $cataloginventory_stock_item_data['is_in_stock'] = 0;
//            }

            $cataloginventory_stock_item_query = $this->createInsertQuery('cataloginventory_stock_item',$cataloginventory_stock_item_data);


            if (Bootstrap::getConfig('dev_mode')) {
                $cataloginventory_stock_item_import = $this->importProductData($cataloginventory_stock_item_query);

                if (!$cataloginventory_stock_item_import) {
                    //warning
                    Bootstrap::logQuery($cataloginventory_stock_item_query);
                    var_dump(1);
                    exit;
                }

            } else {
                $all_query[] = $cataloginventory_stock_item_query;
            }
...
//=================================================================
        ...
        if (isset($convert['tier_prices'])) {
            foreach ($convert['tier_prices'] as $key => $value) {
                $tier_price_website_id = array(0);
                if($value['website_id']){
                    $tier_price_website_id = $this->getWebsiteIdsTargetByIdSrc(array($value['website_id']));
                    if(!$tier_price_website_id){
                        $tier_price_website_id = array(0);
                    }
                }
                foreach ($tier_price_website_id as $key1 => $website_id){
                    $catalog_product_entity_tier_price_data = array(
                        'row_id'         => $product_id,
                        'all_groups'        => $this->getValue($value, 'all_groups', 1),
                        'customer_group_id' => $this->getValue($this->_notice['map']['customer_group'], $this->getValue($value, 'customer_group_id', -1), 0),
                        'qty'               => $value['qty'],
                        'value'             => $value['price'],
                        'website_id'        => $website_id,
                    );
                    if ($this->convertVersion($this->_notice['target']['config']['version'], 2) >= 220) {
                        $catalog_product_entity_tier_price_data['percentage_value'] = null;
                    }
                    $catalog_product_entity_tier_price_query = $this->createInsertQuery('catalog_product_entity_tier_price',$catalog_product_entity_tier_price_data);
                    /**
                     * Start Namlv
                     */

                    if (Bootstrap::getConfig('dev_mode')) {
                        $catalog_product_entity_tier_price_import = $this->importProductData($catalog_product_entity_tier_price_query);

                        if (!$catalog_product_entity_tier_price_import) {
                            //warning
                            Bootstrap::logQuery($catalog_product_entity_tier_price_query);
                            var_dump(1);
                            exit;
                        }

                    } else {
                        $all_query[] = $catalog_product_entity_tier_price_query;
                    }
                }
        ...
        //tier_price end
//=================================================================
        //product_website begin
        $product_website_ids = array();
        if (isset($convert['product_website'])) {
            $product_website_ids = $this->getWebsiteIdsTargetByIdSrc($convert['product_website']);
        }
        foreach ($product_website_ids as $key => $website_id) {
            $catalog_product_website_data = array(
                'product_id' => $product_id,
                'website_id' => $website_id,
            );

            $catalog_product_website_query = $this->createInsertQuery('catalog_product_website',$catalog_product_website_data);
            /**
             * Start Namlv
             */
            if (Bootstrap::getConfig('dev_mode')) {
                $catalog_product_website_import = $this->importProductData($catalog_product_website_query);

                if (!$catalog_product_website_import) {
                    //warning
                    Bootstrap::logQuery($catalog_product_website_query);
                    var_dump(1);
                    exit;
                }

            } else {
                $all_query[] = $catalog_product_website_query;
            ...
            foreach ($convert['options'] as $key => $item) {
                ...
                if ($option_value) {
                    $catalog_product_option_data = array(
                        'product_id'     => $product_id,
                        'type'           => $this->getValue($option_value, 'option_type'),
                        'is_require'     => $this->getValue($option_value, 'option_is_require', 1),
                        'sku'            => $this->getValue($option_value, 'option_sku'),
                        'max_characters' => $this->getValue($option_value, 'option_max_characters'),
                        'file_extension' => $this->getValue($option_value, 'option_max_characters'),
                        'image_size_x'   => $this->getValue($option_value, 'option_image_size_x'),
                        'image_size_y'   => $this->getValue($option_value, 'option_image_size_y'),
                        'sort_order'     => 0,
                    );

                    $catalog_product_option_query = $this->createInsertQuery('catalog_product_option',$catalog_product_option_data);



                    $catalog_product_option_id = $this->importProductData($catalog_product_option_query);
                    if ($catalog_product_option_id) {
                        $option_title = $this->getValue($item, 'title');
                        $option_price = $this->getValue($item, 'price');
                        $option_type  = $this->getValue($item, 'type');
                        if ($option_title) {
                            foreach ($option_title as $key_title => $title) {
                                //catalog_product_option_title begin
                                $catalog_product_option_title_data = array(
                                    'option_id' => $catalog_product_option_id,
                                    'store_id'  => $this->getMapStoreView($this->getValue($title, 'store_id', 0)),
                                    'title'     => $this->getValue($title, 'title'),
                                );

                                $catalog_product_option_title_query = $this->createInsertQuery('catalog_product_option_title',$catalog_product_option_title_data);
                                if (Bootstrap::getConfig('dev_mode')) {
                                    $catalog_product_option_title_import = $this->importProductData($catalog_product_option_title_query);

                                    if (!$catalog_product_option_title_import) {
                                        //warning
                                        var_dump($catalog_product_option_title_query);
                                        exit;

                                    }

                                } else {
                                    $all_query[] = $catalog_product_option_title_query;
                        ...
                        if ($option_price) {
                            foreach ($option_price as $key_price => $price) {
                                //catalog_product_option_price begin
                                $catalog_product_option_price_data = array(
                                    'option_id'  => $catalog_product_option_id,
                                    'store_id'   => $this->getMapStoreView($this->getValue($price, 'store_id', 0)),
                                    'price'      => $this->getValue($price, 'price', '0.0000'),
                                    'price_type' => $this->getValue($price, 'price_type', 'fixed'),
                                );

                                $catalog_product_option_price_query = $this->createInsertQuery('catalog_product_option_price',$catalog_product_option_price_data);

                                if (Bootstrap::getConfig('dev_mode')) {
                                    $catalog_product_option_price_import = $this->importProductData($catalog_product_option_price_query);
                                    if (!$catalog_product_option_price_import) {
                                        Bootstrap::logQuery($catalog_product_option_query);
                                        var_dump(1);
                                        exit;
                                    }
                                } else {
                                    $all_query[] = $catalog_product_option_price_query;
                                }

                                //catalog_product_option_price end
                            ...
                            foreach ($option_type as $key_type => $type) {
                                ...
                                if ($type_value) {
                                    $catalog_product_option_type_value_data = array(
                                        'option_id'  => $catalog_product_option_id,
                                        'sku'        => $this->getValue($type_value, 'sku'),
                                        'sort_order' => $this->getValue($type_value, 'sort_order', 0),
                                    );

                                    $catalog_product_option_type_value_query = $this->createInsertQuery('catalog_product_option_type_value',$catalog_product_option_type_value_data);
                                    ...
                                    if ($catalog_product_option_type_value_id) {
                                        $type_title = $this->getValue($type, 'title');
                                        if ($type_title) {
                                            foreach ($type_title as $key_type_title => $title_type) {
                                                ...
                                                $catalog_product_option_type_title_data = array(
                                                    ...
                                                    'store_id'       => $this->getMapStoreView($this->getValue($title_type, 'store_id')),
                                                    'title'          => $this->getValue($title_type, 'title'),
                                                );

                                                $catalog_product_option_type_title_query  = $this->createInsertQuery('catalog_product_option_type_title',$catalog_product_option_type_title_data);

                                                if (Bootstrap::getConfig('dev_mode')) {
                                                    $catalog_product_option_type_title_import = $this->importProductData($catalog_product_option_type_title_query);
                                                    ...
                                                    $all_query[] = $catalog_product_option_type_title_query;
                                                }

                                                //catalog_product_option_type_title end
                                            }
                                        }
                                        $type_price = $this->getValue($type, 'price');
                                        if ($type_price) {
                                            foreach ($type_price as $key_type_price => $price_type) {
                                                //catalog_product_option_type_price begin
                                                $catalog_product_option_type_price_data = array(
                                                    'option_type_id' => $catalog_product_option_type_value_id,
                                                    'store_id'       => $this->getMapStoreView($this->getValue($price_type, 'store_id', 0)),
                                                    'price'          => $this->getValue($price_type, 'price', '0.0000'),
                                                    'price_type'     => $this->getValue($price_type, 'price_type', 'fixed'),
                                                );

                                                $catalog_product_option_type_price_query = $this->createInsertQuery('catalog_product_option_type_price',$catalog_product_option_type_price_data);


                                                if (Bootstrap::getConfig('dev_mode')) {
                                                    $catalog_product_option_type_price_import = $this->importProductData($catalog_product_option_type_price_query);
...
//                                                        }
                                                    ...
                                                    $all_query[] = $catalog_product_option_type_price_query;
                                                }
...
//=================================================================
        ...
        $tax_class_id = 0;
        if (isset($convert['tax']['id']) && $convert['tax']['id'] != "") {
            $tax_class_id = $this->getProductTaxClassId($convert['tax']['id']);
        }

        $url_key_multistore = array();
        $pro_url_key = $convert['url_key'];
        if ($pro_url_key) {
            $pro_url_key = $this->getProductUrlKey($pro_url_key, 0, $product_eav_attribute_array['url_key']['attribute_id'],$convert['name']);
        }
        $pro_url_path = $this->getProductUrlPath($convert['url_path'],0,$product_eav_attribute_array['url_key']['attribute_id'],$pro_url_key);

        $url_key_multistore[0] = $pro_url_key;
        $insert_attribute_array = array(
            'name'                      => $convert['name'],
            'meta_title'                => $convert['meta_title'],
            'meta_description'          => $convert['meta_description'],
            'image'                     => $image_name ? $image_name : null,
            'small_image'               => $image_name ? $image_name : null,
            'thumbnail'                 => $image_name ? $image_name : null,
            'description'               => $this->changeImgSrcInText($convert['description'],Bootstrap::getConfig('migrate_image_des')),
            'short_description'         => $this->changeImgSrcInText($convert['short_description'],Bootstrap::getConfig('migrate_image_des')),
            'meta_keyword'              => $convert['meta_keyword'],
            'special_price'             => $convert['special_price']['price'],
            'special_from_date'         => $convert['special_price']['start_date'],
            'special_to_date'           => $convert['special_price']['end_date'],
            'length'                    => $convert['length'],
            'width'                     => $convert['width'],
            'height'                    => $convert['height'],
            'price'                     => $convert['price'],
            'weight'                    => $convert['weight'],
            'visibility'                => $this->getValue($convert, 'visibility', 4),
            'quantity_and_stock_status' => $convert['manage_stock'] ? 1 : 0,
            'tax_class_id'              => $tax_class_id ? $tax_class_id : 0,
            'status'                    => $convert['status'],
            'url_key'                   => $pro_url_key,
            'url_path'                  => $pro_url_path,
        );
        if($image_name){
            $insert_attribute_array['image_label'] = $convert['image']['label']?$convert['image']['label']:null;
        }
        if (isset($convert['downloadable'])) {
            $insert_attribute_array['links_title']                = $this->getValue($convert['downloadable'], 'links_title');
            $insert_attribute_array['links_purchased_separately'] = $this->getValue($convert['downloadable'], 'links_purchased_separately');
            $insert_attribute_array['samples_title']              = $this->getValue($convert['downloadable'], 'samples_title');

        }
        if ($convert['type_id'] == "bundle") {
            $insert_attribute_array['price_type']  = $this->getValue($convert, 'price_type');
            $insert_attribute_array['weight_type'] = $this->getValue($convert, 'weight_type');
            $insert_attribute_array['sku_type']    = $this->getValue($convert, 'sku_type');
        }
//        if (isset($convert['parent_configurable']) && count($convert['parent_configurable']) > 0) {
//            $insert_attribute_array['visibility'] = 1;
//        }
//
        //manufacturer begin
        if(isset($convert['manufacturer'])){
            if($convert['manufacturer']['id']){
                $manufacturer_id = $this->getMapFieldBySource(self::TYPE_MANUFACTURER, $convert['manufacturer']['id']);
                if($manufacturer_id){
                    if(isset($product_eav_attribute_array['manufacturer'])){
                        $insert_attribute_array['manufacturer'] = $manufacturer_id;
                    }
                }
            }
        }
        //manufacturer end
        $special_attribute = array(
            'price_type',
            'weight_type',
            'sku_type'
        );
        foreach ($product_eav_attribute_array as $key1 => $value1) {
            foreach ($insert_attribute_array as $key2 => $value2) {
                if (($key2 == $key1)) {
                    ...
                    if(in_array($key2,$special_attribute)){
                    ...
                    $product_attr_query = $this->createInsertQuery("catalog_product_entity_" . $value1['backend_type'],$product_attr_data);
        ...
        if (isset($convert['languages'])) {
            foreach ($convert['languages'] as $key => $value) {
                $insert_attribute_array = array(
                    ...
                    'meta_title'        => $value['meta_title'],
                    'meta_description'  => $value['meta_description'],
                    'description'       => $this->changeImgSrcInText($value['description'],Bootstrap::getConfig('migrate_image_des')),
                    'short_description' => $this->changeImgSrcInText($value['short_description'],Bootstrap::getConfig('migrate_image_des')),
                    'meta_keyword'      => $value['meta_keyword'],
                    'status'            => $value['status'],
                    'url_key'           => $value['url_key'],
                    'url_path'          => $value['url_path'],
                );
//                var_dump($insert_attribute_array);exit;
                foreach ($product_eav_attribute_array as $key1 => $value1) {
                    foreach ($insert_attribute_array as $key2 => $value2) {
                        if (($key2 == $key1)) {
                            $store_id = $this->getMapStoreView($key);
                            if ($key2 == 'url_key') {
                                $value2 = $this->getProductUrlKey($value2, $store_id, $value1['attribute_id'],$value['name']);
                                $url_key_multistore[$store_id] = $value2;
                            }
                            if($key2 == 'url_path'){
                                $value2 = $this->getProductUrlPath($value2,$store_id,$value1['attribute_id']);
                            }
                            if(!$value2){
                                continue;
                            }
        ...
        //insert product attribute end
//=================================================================
        //configurable product begin
        $configurable_product_parent_id = null;

        //link configurable product and simple product begin
        if (isset($convert['parent_configurable']) && isset($convert['parent_configurable'][0]['id'])) {
            $configurable_product_queries = array();
            foreach ($convert['parent_configurable'] as $key => $item) {
                $configurable_product_parent_import = $this->_importProductParent($item);
...
//                    return $response;
                ...
                if ($configurable_product_parent_id) {
                    // link configurable product and simple product begin
...
//                    $configurable_product_queries['catalog_product_configurable_relation_query_' . $key] = array(
//                        'type'   => 'insert',
//                        'query'  => $catalog_product_relation_query,
//                    );
//                    $catalog_product_relation_import = $this->getConnectorData($url_query, array(
//                        'query' => serialize(array(
//                            'type'   => 'insert',
//                            'query'  => $catalog_product_relation_query,
//                            'params' => array(
...
//                    }
//
//                    if ($catalog_product_relation_import['result'] != 'success') {
//                        //warning
//                        return $this->warningSQL($catalog_product_relation_import);
//                    }
                    //catalog_product_relation end

                    //catalog_product_super_link begin
                    $catalog_product_super_link_data = array(
                        'product_id' => $product_id,
                        'parent_id'  => $configurable_product_parent_id,
                    );

                    $catalog_product_super_link_query = $this->createInsertQuery('catalog_product_super_link',$catalog_product_super_link_data);

                    if (Bootstrap::getConfig('dev_mode')) {
                        $catalog_product_super_link_import = $this->importProductData($catalog_product_super_link_query);

                        if (!$catalog_product_super_link_import) {
                            //warning
                            Bootstrap::logQuery($catalog_product_super_link_query);
                            var_dump(1);
                            exit;
                        }

                    } else {
                        $all_query[] = $catalog_product_super_link_query;
                    }

                    //catalog_product_super_link end
                    //link configurable product and simple product end
                }
            }
//            if (count($configurable_product_queries)) {
//                $configurable_product_import = $this->getConnectorData($url_query, array(
//                    'serialize' => true,
//                    'query'     => serialize($configurable_product_queries),
//                ));
//                if (!$configurable_product_import) {
//                    //warning
//                    var_dump($configurable_product_queries);
//
//                    return $this->errorConnector();
//                }
//
//                if ($configurable_product_import['result'] != 'success') {
//                    //warning
//                    return $this->warningSQL($configurable_product_import);
//                }
//            }
        ...
        //insert attribute product begin
        if (isset($convert['attribute_configurable'])) {
            foreach ($convert['attribute_configurable'] as $key => $item) {
                $attribute_exists = false;
                $attribute_id     = null;
                foreach ($product_eav_attribute_array as $key1 => $value) {
                    if ($item['attribute_code'] == $key1) {
                ...
                if (!$attribute_exists) {
                    // $attribute_id = $this->createAttribute($item['attribute_code'],'int','select',$item['frontend_label'],$convert['attribute_set_id']);
                    //eav_attribute begin
                    $eav_attribute_data = array(
                        'entity_type_id'  => 4,
                    ...
                    $eav_attribute_query  = $this->createInsertQuery('eav_attribute',$eav_attribute_data);
                    $attribute_id = $this->importProductData($eav_attribute_query);
                    ...
                    if (!$attribute_id ) {
                        ...
                        //eav_attribute end
                        $eav_group_name   = isset($eav_attribute_group[$item['attribute_id']]) ? $eav_attribute_group[$item['attribute_id']] : null;
                        $attribute_set_id = $this->_notice['map']['attributes'][$convert['attribute_set_id']];
                        //eav_entity_attribute begin
                        $eav_entity_attribute_data = array(
                            'entity_type_id'     => 4,
                            'attribute_set_id'   => $attribute_set_id,
                            'attribute_group_id' => $this->getAttributeGroupId($attribute_set_id, $eav_group_name),
                            'attribute_id'       => $attribute_id,
                            'sort_order'         => 15,
                        );

                        $eav_entity_attribute_query  = $this->createInsertQuery('eav_entity_attribute',$eav_entity_attribute_data);
                        if(Bootstrap::getConfig('dev_mode')){
                            $eav_entity_attribute_import = $this->importProductData($eav_entity_attribute_query);

                            if (!$eav_entity_attribute_import) {
                                //warning
//                                if (Bootstrap::getConfig('dev_mode')) {
                                Bootstrap::logQuery($eav_entity_attribute_query);
                                var_dump(1);
                                exit;
//                                }
...
//                                return $this->errorConnector();
                            ...
                            $all_query[] = $eav_entity_attribute_query;
                        }
                        ...
                        $catalog_eav_attribute_query  = $this->createInsertQuery('catalog_eav_attribute',$catalog_eav_attribute_data);
                        if(Bootstrap::getConfig('dev_mode')){
                            $catalog_eav_attribute_import = $this->importProductData($catalog_eav_attribute_query);
...
//                            return $this->errorConnector();
                            ...
                            $all_query[] = $catalog_eav_attribute_query;
                        }
                ...
                //catalog_product_super_attribute begin
                $catalog_product_super_attribute_data = array(
                    'product_id'   => $product_id,
                    'attribute_id' => $attribute_id,
                    'position'     => 0,
                );

                $catalog_product_super_attribute_query  = $this->createInsertQuery('catalog_product_super_attribute',$catalog_product_super_attribute_data);
                $product_super_attribute_id = $this->importProductData($catalog_product_super_attribute_query);

                if (!$product_super_attribute_id) {
                    //warning
                    if (Bootstrap::getConfig('dev_mode')) {
                        Bootstrap::logQuery($catalog_product_super_attribute_query);
                        var_dump(1);
                        exit;
                    }
...
//                        return $this->errorConnector();
                    ...
                    $catalog_product_super_attribute_label_data = array(
                        'product_super_attribute_id' => $product_super_attribute_id,
                        // 'store_id ' => 0,
                        // 'use_default ' => 0,
                        'value'                      => $item['super_attribute_label'] ? $item['super_attribute_label'] : null,
                    );

                    $catalog_product_super_attribute_label_query  = $this->createInsertQuery('catalog_product_super_attribute_label',$catalog_product_super_attribute_label_data);
                    if(Bootstrap::getConfig('dev_mode')){
                        $catalog_product_super_attribute_label_import = $this->importProductData($catalog_product_super_attribute_label_query);

                        if (!$catalog_product_super_attribute_label_import) {
                            //warning
//                            if (Bootstrap::getConfig('dev_mode')) {
                            Bootstrap::logQuery($catalog_product_super_attribute_label_query);
                            var_dump(1);
                            exit;
//                            }
...
//                            return $this->errorConnector();
                        ...
                        $all_query[] = $catalog_product_super_attribute_label_query;
                    }

                }

                //catalog_product_super_attribute_label end


                if (isset($item['option_data']) && is_array($item['option_data'])) {
                    $option_data_array = array();
                    $option_data_array = $item['option_data'];
                    foreach ($option_data_array as $key2 => $value) {
                        $option_exists = $this->selectMap($url_src, $url_desc, self::TYPE_OPTION, $value['option_id'], null, $value['option_value']);
                        ...
                        if ($option_exists) {
                            ...
                            $eav_attribute_option_query  = $this->createInsertQuery('eav_attribute_option',$eav_attribute_option_data);
                            $option_id = $this->importProductData($eav_attribute_option_query);

                            if ($option_id) {

                                $this->insertMap($url_src, $url_desc, self::TYPE_OPTION, $value['option_id'], $option_id, $value['option_value']);
                                //eav_attribute_option end
                                ...
                                $eav_attribute_option_value_query  = $this->createInsertQuery('eav_attribute_option_value',$eav_attribute_option_value_data);
                                $eav_attribute_option_value_import = $this->importProductData($eav_attribute_option_value_query);
...
//        ===============================================================
        ...
        if (isset($convert['parent_bundle']) && isset($convert['parent_bundle'][0]['id'])) {
            foreach ($convert['parent_bundle'] as $key => $bundle) {
                $bundle_parent_import = $this->_importProductParent($bundle);
                if ($bundle_parent_import['result'] != 'success') {
                    $response           = $this->_defaultResponse();
                    $response['result'] = 'warning';
                    $response['msg']    = $this->consoleWarning('Could not import');
                ...
                $bundle_parent_id = $bundle_parent_import['data'];
                if ($bundle_parent_id) {
                    $catalog_product_relation_data  = array(
                    ...
                    if (isset($bundle['bundle_selection'])) {
                        $product_src      = $this->selectMap($url_src, $url_desc, self::TYPE_PRODUCT, null, $product_id);
                        $product_id_src   = $product_src['id_src'];
                        $bundle_selection = $this->getListFromListByField($bundle['bundle_selection'], 'product_id', $product_id_src);
                        foreach ($bundle_selection as $key_selection => $value_selection) {
                            $catalog_product_bundle_selection_id_query = $this->createInsertQuery('sequence_product_bundle_selection',array('sequence_value' => ''));
                            $selection_id = $this->importProductData($catalog_product_bundle_selection_id_query);
                            if(!$selection_id){
                                continue;
                            }
                            $bundle_option_map = $this->selectMap($url_src, $url_desc, self::TYPE_BUNDLE_OPTION, $value_selection['option_id']);
                            if ($bundle_option_map && $bundle_option_map['id_desc'] > 0) {
                                $bundle_option_id                       = $bundle_option_map['id_desc'];
                                $catalog_product_bundle_selection_data  = array(
                                    'selection_id' => $selection_id,
                                    'option_id'                => $bundle_option_id,
                                    'parent_product_id'        => $bundle_parent_id,
                                    'product_id'               => $product_id,
                                    'position'                 => $this->getValue($value_selection, 'position', 0),
                                    'is_default'               => $this->getValue($value_selection, 'is_default', 0),
                                    'selection_price_type'     => $this->getValue($value_selection, 'selection_price_type', 0),
                                    'selection_price_value'    => $this->getValue($value_selection, 'selection_price_value', '0.0000'),
                                    'selection_qty'            => $this->getValue($value_selection, 'selection_qty'),
                                    'selection_can_change_qty' => $this->getValue($value_selection, 'selection_can_change_qty', 0),
                                );
                                $catalog_product_bundle_selection_query = $this->createInsertQuery('catalog_product_bundle_selection',$catalog_product_bundle_selection_data);

                                if (Bootstrap::getConfig('dev_mode')) {
                                    $catalog_product_bundle_selection_import = $this->importProductData($catalog_product_bundle_selection_query);
                                    if (!$catalog_product_bundle_selection_import) {
                                        Bootstrap::logQuery($catalog_product_bundle_selection_query);
                                        var_dump(1);
                                        exit;
                                    }
                                } else {
                                    $all_query[] =$catalog_product_bundle_selection_query;
                                }
                            }
        ...
        if (isset($convert['bundle_option'])) {
            //bundle option
            foreach ($convert['bundle_option'] as $key_option => $values_option) {
                $catalog_product_bundle_option_id_query = $this->createInsertQuery('sequence_product_bundle_option',array('sequence_value' => ''));
                $bundle_option_id = $this->importProductData($catalog_product_bundle_option_id_query);
                if(!$bundle_option_id){
                    continue;
                }
                $this->insertMap($url_src, $url_desc, self::TYPE_BUNDLE_OPTION, $values_option['option']['option_id'], $bundle_option_id);
                $catalog_product_bundle_option_data   = array(
                    'option_id' => $bundle_option_id,
                    'parent_id' => $product_id,
                    'required'  => $this->getValue($values_option['option'], 'required', 0),
                    'position'  => $this->getValue($values_option['option'], 'position', 0),
                    'type'      => $this->getValue($values_option['option'], 'type', null),
                );
                $catalog_product_bundle_option_query  = $this->createInsertQuery('catalog_product_bundle_option',$catalog_product_bundle_option_data);
                if (Bootstrap::getConfig('dev_mode')) {
                    $catalog_product_bundle_option_value_import = $this->importProductData($catalog_product_bundle_option_id_query);
                    if (!$catalog_product_bundle_option_value_import) {
                        Bootstrap::logQuery($catalog_product_bundle_option_query);
                        var_dump(1);
                        exit;
                    }
                } else {
                    $all_query[]=$catalog_product_bundle_option_query;
                }
//                $bundle_option_id = $this->importProductData($catalog_product_bundle_option_query);
//                if (!$bundle_option_id ) {
//                    //warning
//                    if(Bootstrap::getConfig('dev_mode')){
//
//                        var_dump($catalog_product_bundle_option_query);
//                        exit;
//                    }
//                    continue;
//                }

                foreach ($values_option['value'] as $key_value => $value_values) {
                    $catalog_product_bundle_option_value_data  = array(
                        'option_id' => $bundle_option_id,
                        'store_id'  => $this->getMapStoreView($this->getValue($value_values, 'store_id', 0)),
                        'title'     => $this->getValue($value_values, 'title'),
                    );
                    if ($this->convertVersion($this->_notice['target']['config']['version'], 2) >= 220) {
                        $catalog_product_bundle_option_value_data['parent_product_id'] = $product_id;
                    }
                    $catalog_product_bundle_option_value_query = $this->createInsertQuery('catalog_product_bundle_option_value',$catalog_product_bundle_option_value_data);

                    if (Bootstrap::getConfig('dev_mode')) {
                        $catalog_product_bundle_option_value_import = $this->importProductData($catalog_product_bundle_option_value_query);
                        if (!$catalog_product_bundle_option_value_import) {
                            Bootstrap::logQuery($catalog_product_bundle_option_value_query);
                            var_dump(1);
                            exit;
                        }
                    } else {
                        $all_query[]=$catalog_product_bundle_option_value_query;
                    }
                }
//
...
//            var_dump($convert['bundle_option']);exit;
        }
...
//=================================================================|||
        ...
        if (isset($convert['product_link_parent'])) {
            foreach ($convert['product_link_parent'] as $key => $value) {
                ...
                if ($children && isset($children['id_desc']) && $children['id_desc'] > 0) {
                    ...
                    $catalog_product_link_relate_query  = $this->createInsertQuery('catalog_product_link',$catalog_product_link_relate_data);
                    if(Bootstrap::getConfig('dev_mode')){
         ...
         * End Namlv
         */
        // product relate, up-sell,cross-sell finish
//=================================================================|||
        //insert remain attribute begin
//        $product_eav_attribute_array = array();
//        foreach ($product_eav_attribute['data']['eav_attribute'] as $attribute) {
//            $product_eav_attribute_array[$attribute['attribute_code']]['attribute_id'] = $attribute['attribute_id'];
//            $product_eav_attribute_array[$attribute['attribute_code']]['backend_type'] = $attribute['backend_type'];
            ...
            foreach ($convert['add_data'] as $key => $value) {
                ...
                if (!$attribute_id) {
                    ...
                    $eav_attribute_data = array(
                        ...
                        'frontend_label'  => $value['frontend_label'],
                        'frontend_class'  => null,
                        'source_model'    => null,
                        'is_required'     => 0,
                        'is_user_defined' => 1,
                    ...
                    $this->insertMap($url_src, $url_desc, self::TYPE_ATTR, $value['attribute_id'], $attribute_id, $value['attribute_code']);
                    //eav_attribute end

                    $eav_group_name   = isset($eav_attribute_group[$value['attribute_id']]) ? $eav_attribute_group[$value['attribute_id']] : null;
                    $attribute_set_id = $this->_notice['map']['attributes'][$convert['attribute_set_id']];
                    //eav_entity_attribute begin
                    $eav_entity_attribute_data = array(
                        'entity_type_id'     => 4,
                ...
                if ($value['frontend_input'] == 'multiselect' || $value['frontend_input'] == 'select') {
                    if (isset($value['option_data'])) {
                        foreach ($value['option_data'] as $key1 => $item) {
                            //check option exists begin
                            $check_option = $this->checkOptionExists($item['value'], $value['attribute_code']);
                            if ($check_option) continue;
                            //check option exists end
                            ...
                            if (!$option_id) {
                                // warning
                                if(Bootstrap::getConfig('dev_mode')){
                                    var_dump($eav_attribute_option_query);exit;
                                }
                                continue;
                            }

                            $this->insertMap($url_src, $url_desc, self::TYPE_OPTION, $item['option_id'], $option_id, $item['value']);
                            //eav_attribute_option end
                ...
                if (isset($value['product_value'])) {
                    if ($value['frontend_input'] == 'select') {
                        // $option_exists = $this->selectMap($url_src, $url_desc, self::TYPE_OPTION, $value['product_value'], null, null);
                        $check_option = $this->checkOptionExists($value['product_value'], $value['attribute_code']);
                        if ($check_option) {
                            $option_id = $check_option;
                            ...
                            $catalog_product_entity_int_query = $this->createInsertQuery('catalog_product_entity_int',$catalog_product_entity_int_data);

                            if (Bootstrap::getConfig('dev_mode')) {
                                $catalog_product_entity_int_import = $this->importProductData($catalog_product_entity_int_query);

                                if (!$catalog_product_entity_int_import) {
                                    Bootstrap::logQuery($catalog_product_entity_int_query);
                                    var_dump(1);
                                    exit;
                                }
                            } else {
                                $all_query[] =$catalog_product_entity_int_query;
                            }

                        }
                    } elseif ($value['frontend_input'] == 'multiselect') {
                        $new_data_array = array();
                        $new_data       = null;
                        $old_data       = explode(',', $value['product_value']);
                        if (is_array($old_data)) {
                            foreach ($old_data as $item1) {
                                // $option_exists = $this->selectMap($url_src, $url_desc, self::TYPE_OPTION, $item1, null, null);
                                $check_option = $this->checkOptionExists($item1, $value['attribute_code']);
                                if ($check_option) {
                                    $new_data_array[] = $check_option;
                                }
                            }
                            $new_data = implode(',', $new_data_array);
                        }
                        if ($new_data) {
                            $catalog_product_entity_varchar_data = array(
                                'attribute_id' => $attribute_id,
                                'store_id'     => 0,
                                'row_id'    => $product_id,
                                'value'        => $new_data,
                            ...
                            $catalog_product_entity_varchar_query = $this->createInsertQuery('catalog_product_entity_varchar',$catalog_product_entity_varchar_data);

                            if (Bootstrap::getConfig('dev_mode')) {
                                $catalog_product_entity_varchar_import = $this->importProductData($catalog_product_entity_varchar_query);

                                if (!$catalog_product_entity_varchar_import) {
                                    Bootstrap::logQuery($catalog_product_entity_varchar_query);
                                    var_dump(1);
                                    exit;
                                }
                            } else {
                                $all_query[] = $catalog_product_entity_varchar_query;
                            }

                        }
                    } else {
                        $entity_table = 'catalog_product_entity_' . $value['backend_type'];
                        $product_value = $value['product_value']?$value['product_value']:null;
//                        if($value['backend_type'] == 'int'){
//                            $product_value = 0;
//                        }
                        ...
                        $catalog_product_entity_int_query = $this->createInsertQuery($entity_table,$catalog_product_entity_int_data);
...
//url_rewrite with no category_id
        if(isset($convert['url_rewrite_product'])){
            $rewrite_product = $convert['url_rewrite_product'];
            $store_id    = $this->getMapStoreView($this->getValue($rewrite_product, 'store_id', 0));
            $target_path = 'catalog/product/view/id/' . $product_id;
            $request_path      = $this->getRequestPath('product', $rewrite_product['request_path'], $store_id);
            $url_rewrite_product_data  = array(
                'entity_type'      => 'product',
                'entity_id'        => $product_id,
            ...
            $url_rewrite_product_query = $this->createInsertQuery('url_rewrite',$url_rewrite_product_data);
            $this->importProductData($url_rewrite_product_query);
        }


        //url_rewrite with category
        $url_rewrite = $convert['url_rewrite'];
        $target_path = null;
        foreach ($url_rewrite as $key => $rewrite) {
            ...
            $url_rewrite_data  = array(
                ...
                'description'      => $this->getValue($rewrite, 'description'),
                'is_autogenerated' => 1,
                'metadata'         => $this->mySerialize($metadata),
            );
            $url_rewrite_query = $this->createInsertQuery('url_rewrite',$url_rewrite_data);
            $url_rewrite_id = $this->importProductData($url_rewrite_query);
            if(!$url_rewrite_id){
                continue;
            }
            $catalog_product_url_rewrite_data = array(
                'url_rewrite_id' => $url_rewrite_id,
                'category_id' => $category_id,
                'product_id' => $product_id,
            );
            $catalog_product_url_rewrite_query = $this->createInsertQuery('catalog_url_rewrite_product_category',$catalog_product_url_rewrite_data);
            if (Bootstrap::getConfig('dev_mode')) {
                $catalog_product_url_rewrite_import = $this->importProductData($catalog_product_url_rewrite_query);
                if (!$catalog_product_url_rewrite_import) {
                    Bootstrap::logQuery($url_rewrite_query);
                    var_dump(1);
                    exit;
                }
            } else {
                $all_query[] = $catalog_product_url_rewrite_query;
            }
        }
...
//      -------------------------------------------------------------------------------------------
    ...
    public function afterReviewImport($review_id, $convert, $review, $reviewsExt)
    {
        ...
        $review_detail_data = array(
            'review_id'   => $review_id,
            'store_id'    => $this->getMapStoreView($this->getValue($convert,'detail_store')),
            'title'       => $convert['title'],
...
//        }
    ...
    public function getRulesExtExport($rules)
    {
        ...
        $row_ids           = $this->duplicateFieldValueFromList($rules['data'], 'row_id');
        $rule_idCon         = $this->arrayToInCondition($rule_ids);
        $row_idCon         = $this->arrayToInCondition($row_ids);
        $rulesExt           = $this->getConnectorData($url_query, array(
            ...
            'query'     => serialize(array(
                ...
                'salesrule_customer_group'    => array(
                    'type'  => 'select',
                    'query' => 'SELECT * FROM _DBPRF_salesrule_customer_group WHERE row_id IN ' . $row_idCon,
                ),
...
//        elseif(isset($rulesExt['data']['salesrule_customer_group'])){
        $customer_groups    = $this->getListFromListByField($rulesExt['data']['salesrule_customer_group'], 'row_id', $rule['row_id']);
        $customer_group_ids = array();
...
//        var_dump($rule_data);exit;
    ...
    public function ruleImport($convert, $rule, $rulesExt)
    {
        $url_src     = $this->_notice['src']['cart_url'];
        $url_desc    = $this->_notice['target']['cart_url'];
        $insert_query = $this->createInsertQuery('sequence_salesrule',array('sequence_value'=>''));
        $rule_id = $this->importCartRuleData($insert_query);
        if(!$rule_id){
            return $this->errorConnector();
        }
...
//        print_r($rule_query);exit;
    ...
    public function getCartrulesExtExport($cartrules)
    {
        ...
        $row_ids          = $this->duplicateFieldValueFromList($cartrules['data'], 'row_id');
        $rule_id_con       = $this->arrayToInCondition($rule_ids);
        $row_id_con       = $this->arrayToInCondition($row_ids);
        $cartrules_queries = array(
            'catalogrule_customer_group' => array(
                'type'  => 'select',
                'query' => 'SELECT * FROM _DBPRF_catalogrule_customer_group WHERE row_id IN ' . $row_id_con,
            ),
...
//        $cartrule_customer_group = $this->getListFromListByField($cartrulesExt['data']['catalogrule_customer_group'],'rule_id',$id);
        ...
        $cartrule_customer_group         = $this->getListFromListByField($cartrulesExt['data']['catalogrule_customer_group'], 'row_id', $cartrule['row_id']);
        $customer_group_ids              = array();
...
//        }
    ...
    public function cartruleImport($convert, $cartrule, $cartrulesExt)
    {
        $url_src    = $this->_notice['src']['cart_url'];
        $url_desc   = $this->_notice['target']['cart_url'];
        $insert_query = $this->createInsertQuery('sequence_catalogrule',array('sequence_value'=>''));
        $cartrule_id = $this->importCartRuleData($insert_query);
        if(!$cartrule_id){
            return $this->errorConnector();
        }
        $conditions = $this->getValue($convert, 'conditions');
...
//        print_r($type);exit;
    ...
    function updateSeoPrd($product,$productsExt,$convert){
        $id_src = $product['entity_id'];
        $product_id = $this->getMapFieldBySource(self::TYPE_PRODUCT, $id_src);
        if(!$product_id){
            return;
        }
        $all_query = array();
        $url_query = $this->getConnectorUrl('query');
        $query_delete = "delete from _DBPRF_url_rewrite where entity_type ='product' and entity_id = '".$product_id."'";
        $this->getConnectorData($url_query,array(
            'query' => serialize(array(
                'type' => 'query',
                'query' => $query_delete,
        ...
        if (!Bootstrap::getConfig('dev_mode')) {
            $this->importMultipleData($all_query,'product');
        }
    }
